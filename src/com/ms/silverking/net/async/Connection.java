...
public abstract class Connection implements ChannelRegistrationWorker, Comparable<Connection> {
	...
	public void setConnectionListener(ConnectionListener connectionListener) {
	    if (this.connectionListener != null) {
	        Log.warning("Ignoring attempt to reset connection listener: ", this);
	    }  else {
	        this.connectionListener = connectionListener;
	    }
	}
	...
	public final void writeAllPending() throws IOException {
	    if (AsyncGlobals.debug) {
	        Log.fine("Connection.writeAllPending");
	    }
	    if (!connected) {
	        Log.fine("writeAllPending() not connected");
	        return;
	    }
        ...
        if (AsyncGlobals.debug) {
            Log.fine("out Connection.writeAllPending");
        }
	}
	 ...
	 * Called by user threads to send data on this channel asynchronously.
	 */
	public abstract void sendAsynchronous(Object data, UUIDBase sendID, 
	 ...
	 * @throws IOException
	 */
	public void sendSynchronous(Object data, UUIDBase sendID, 
			AsyncSendListener asyncSendListener, long deadline) throws IOException {
	...
	public final int read() throws IOException {
        ...
        if (AsyncGlobals.debug && debug && remoteSocketAddress.equals(InetAddress.getLocalHost())) {
            Log.fine("channelReceiveLock.lock();");
        }
	...
	public void start() {
		selectorController.addKeyChangeRequest(
				new NewKeyChangeRequest(channel, KeyChangeRequest.Type.ADD_AND_CHANGE_OPS,
	...
	protected final void disableReads() {
	    if (connected) {
	        ...
	        Log.warning("Ignoring disconnected disable reads");
	    }
	}	
	...
	protected final void enableReads() {
	    if (connected) {
	        ...
	        Log.warning("Ignoring disconnected enable reads");
	    }
	}
	...
	protected final void enableWrites() {
        if (connected) {
            ...
            Log.warningAsync("Ignoring disconnected enable writes");
        }
	}
	...
	protected final void enableWritesAsync() {
        if (connected) {
            ...
            Log.warning("Ignoring disconnected enable writes");
        }
	}
	*/
	...
	void disconnect() {
		...
		try {
			...
			try {
				...
				Log.warning(ioe +" "+ remoteSocketAddress);
			}
	...
	protected void sendFailed(OutgoingData data) {
		...
		if (activeSend != null) {
			...
			if (Log.levelMet(Level.FINE)) {
				Log.fine("sendFailed() not an active send ", 
						data.getSendUUID() +" "+ remoteSocketAddress);
			}
		}
		data.failed();
	}
	...
	protected void sendTimedOut(OutgoingData data) {
		...
		if (activeSend != null) {
			...
			if (Log.levelMet(Level.FINE)) {
				Log.fine("sendTimedOut() not an active send ", 
						data.getSendUUID() +" "+ remoteSocketAddress);
			}
		}
		data.timeout();
	...
	protected void sendSucceeded(OutgoingData data) {
		...
		if (activeSend != null) {
			...
			if (AsyncGlobals.debug && debug && Log.levelMet(Level.FINE)) {
				Log.fine("sendSucceeded() not an active send ", 
						data.getSendUUID() +" "+ remoteSocketAddress);
			}
		}
		data.successful();
	...
	public void channelRegistered(SelectionKey key) {
		synchronized (this) {
			...
			if (this.selectionKey != null) {
				Log.warning("Duplicate channel registration!");
			}
