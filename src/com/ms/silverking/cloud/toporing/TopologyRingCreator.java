...
public class TopologyRingCreator {
    ...
    private static final String magnitudeToleranceFactorProperty = TopologyRingCreator.class.getName() +".MagnitudeToleranceFactor";
    private static final int	defaultMagnitudeToleranceFactor = 1;
    private static final int	magnitudeToleranceFactor;
    static {
    	magnitudeToleranceFactor = PropertiesHelper.systemHelper.getInt(magnitudeToleranceFactorProperty, defaultMagnitudeToleranceFactor, ParseExceptionAction.RethrowParseException);
    }
    ...
    public TopologyRing create(RingTreeRecipe recipe, String nodeID) {
        ...
        if (node != null) {
            ...
            sourceRing = SingleRing.emptyRing(
                    recipe.topology.getNodeByID(nodeID).getChildNodeClass(), 0,
     ...
     */
    private TopologyRing _create2(SingleRing sourceRing, RingTreeRecipe recipe,
            String ringParentID) {
        /*
        ...
        for (SubPolicy subPolicy : recipe.storagePolicy.getSubPolicies()) {
            allocateSubPolicy2(sourceRing, prList, subPolicy, ringParentID,
                    recipe);
        }
    ...
    private void allocateSubPolicy2(SingleRing sourceRing,
            ProtoRegionList prList, SubPolicy subPolicy, String ringParentID,
        ...
        for (SubPolicyMember member : subPolicy.getMembers()) {
            allocateSubPolicyMember2(sourceRing, prList, member, recipe,
                    parent, subPolicy.getReplicationType());
        }
    }
    ...
    private int nextRegion(ProtoRegionList prList,
            RegionAllocationMode regionAllocationMode, SingleRing sourceRing,
            int regionIndex, Node node, int maxOwners, ReplicationType rType,
        ...
        if (debug) {
            System.out
                    .printf("TopologyRingCreator.nextRegion regionIndex %d maxOwners %d\n",
                            regionIndex, maxOwners);
            System.out
                    .printf("regionAllocationMode %s\n", regionAllocationMode);
        }
        minPotentialOwners = Integer.MAX_VALUE;
        bestCandidateIndex = -1;
        RingInteger _candidateRegionIndex = new RingInteger(prList.size() - 1,
                lastCandidateValue);
        for (int ii = 0; ii < prList.size(); ii++) {
            ...
            if (debug) {
                System.out
                        .printf("TopologyRingCreator.nextRegion loop candidateRegionIndex %d\n",
            ...
            if (!candidateRegion.contains(node)
                    && candidateRegion.getOwners(rType).size() < maxOwners) {
                if (debug) {
                    System.out.printf("candidateRegion %s\n", candidateRegion);
                }
                switch (regionAllocationMode) {
                case Primary:
                    valid = sourceRing.pointOwnedByNode(candidateRegion
                            .getRegion().getStart(), node,
                ...
                if (valid) {
                    ...
                    potentialOwnersSet
                            .removeAll(candidateRegion.getOwnersSet());
    ...
    private SwapResult swap(ProtoRegionList prList, Node newNode, 
                            int ownedIndex, int nonownedIndex, long maxAllocation) {
        Node        existingNode;
     ...
     * @return
     */
    private int createNextRegion(ProtoRegionList prList, Node node,
            ReplicationType rType, int maxOwners, long swapRemaining) {
        ...
        //while (swapped < swapRemaining) {
            ...
            if (ownedRegion < 0) {
                Log.warning("Couldn't find ownedRegion "+ node +" "+ prList.toString());
            }
                    ...
                    if (!candidateRegion.contains(node)) {
                        if (candidateRegion.getOwners(rType).size() < maxOwners) {
                            ...
                            throw new RuntimeException(
                                    "Unexpectedly found a free region");
                        } else {
            ...
            if (nonownedRegion < 0) {
                Log.warning("Couldn't find nonownedRegion "+ node +" "+ prList.toString());
            }
    ...
    static class NASAvailableRingspaceComparator implements
            Comparator<NodeAllocationState> {
        NASAvailableRingspaceComparator() {
        }

        @Override
        public int compare(NodeAllocationState nas0, NodeAllocationState nas1) {
            return Long.compare(nas0.availableRingspace(),
                    nas1.availableRingspace());
        }
    }

    private List<RingRegion> fullRegions;
    private static final NASAvailableRingspaceComparator nasAvailableRingspaceComparator = new NASAvailableRingspaceComparator();
    ...
    class NodeAllocationState {
        ...
        void addAllocation(int replica, RingRegion region) {
            ...
            if (allocated[replica] + allocation > NumUtil.addWithClamp(toAllocate, magnitudeTolerance)) {
                throw new RuntimeException("Excessive allocation: " + node
                        + " " + replica + " " + allocated[replica] + "\t"
                        + toAllocate);
            }
        ...
        void addAllocation(int replica, List<RingRegion> regions) {
            ...
            if (allocated[replica] + allocation > NumUtil.addWithClamp(toAllocate, magnitudeTolerance)) {
                throw new RuntimeException("Excessive allocation: " + node
                        ...
                        + " allocated[replica]: " + allocated[replica]
                        + "\ttoAllocate: "+ toAllocate 
                        +"\t allocated+allocation: "+ (allocated[replica] + allocation)
                        +"\t toAllocate+magnitudeTolerance "+ (toAllocate + magnitudeTolerance)
                        +"\t NumUtil.addWithClamp(toAllocate, magnitudeTolerance) "+ NumUtil.addWithClamp(toAllocate, magnitudeTolerance));
            }
            allocated[replica] += allocation;
    ...
    private List<NodeAllocationState> createNodeAllocationStates(
            SubPolicyMember member, RingTreeRecipe recipe, Node parent,
            int replicas) {
        ...
        nodeClass = parent.getChildNodeClass();
        if (nodeClass == null) {
            System.err.println("parent.getChildren().size(): "
        ...
        for (int i = 0; i < nodes.size(); i++) {
            nodeAllocationStates.add(new NodeAllocationState(nodes.get(i),
                    replicas, nodeRegionSizes.get(i)));
        }
    ...
    private List<NodeAllocationState> getInitialAllocationStates(
        ...
        nodeAllocationStates_initial = new ArrayList<>(
                nodeAllocationStates_final.size());
        for (int i = 0; i < nodeAllocationStates_final.size(); i++) {
            long oldAllocation;
            long allocationLimit;

            oldAllocation = sourceRing.getOwnedRingspace(
                    nodeAllocationStates_final.get(i).getNode(), oqm);
            allocationLimit = oldAllocation / replicas;
            if (debug) {
                System.out.printf(
                        "node %s\toldAllocation %d\tallocationLimit %d\n",
                        nodeAllocationStates_final.get(i).getNode(),
                        oldAllocation, allocationLimit);
            }
            nodeAllocationStates_initial.add(nodeAllocationStates_final.get(i)
                    .limitAllocation(allocationLimit));
        }
        return nodeAllocationStates_initial;
    }

    private void allocateSubPolicyMember2(SingleRing sourceRing,
            ProtoRegionList prList, SubPolicyMember member,
        ...
        if (debug) {
            System.out
                    .println("TopologyRingCreator.allocateSubPolicyMember2()");
        }
        ...
        nodeAllocationStates_final = createNodeAllocationStates(member, recipe,
                parent, replicas);
        nodeAllocationStates_initial = getInitialAllocationStates(
                nodeAllocationStates_final, sourceRing, rType, replicas);
        doAllocation(sourceRing, prList, rType, replicas, allReplicasTag,
        ...
        for (int i = 0; i < nodeAllocationStates_final.size(); i++) {
            for (int j = 0; j < replicas; j++) {
                nodeAllocationStates_final.get(i).addAllocation(
                        j,
                        nodeAllocationStates_initial.get(i)
                                .getAllocatedRegions(j));
            }
        }
        doAllocation(sourceRing, prList, rType, replicas, allReplicasTag,
    ...
    private void doAllocation(SingleRing sourceRing, ProtoRegionList prList,
            ReplicationType rType, int replicas, boolean allReplicasTag,
            List<NodeAllocationState> nodeAllocationStates) {
        if (debug) {
            System.out.println("\n\ndoAllocation()");
        }
         ...
         */
        for (RegionAllocationMode regionAllocationMode : RegionAllocationMode
                .values()) {
            for (int replicaIndex = 0; replicaIndex < replicas; replicaIndex++) {
                ...
                while (unallocated.size() > 0) {
                    ...
                    if (debug) {
                        System.out
                                .printf("\n\nregionAllocationMode %s replicaIndex %d nodeIndex %d\tnode %s availableRingspace %d\n",
                                        regionAllocationMode, replicaIndex,
                                        nodeIndex, node,
                    ...
                    while (nodeSearchValid
                        ...
                        if (!allReplicasTag) {
                            regionIndex = nextRegion(prList,
                                    regionAllocationMode, sourceRing,
                                    regionIndex, node, replicas, rType,
                                    remainingNodes);
                        } else {
                        ...
                        if (regionIndex < 0) {
                            if (allReplicasTag) {
                                ...
                                case Secondary:
                                    ...
                                    continue;
                                case Any:
                                    regionIndex = createNextRegion(prList, node, rType, replicas, 
                                                                   nas.allocationRemaining(replicaIndex));
                                    break;
                                default:
                                    throw new RuntimeException("panic");
                        ...
                        if (debug) {
                            ...
                            System.out.printf("%d %d\n", protoRegion
                                    ...
                                    nas.ringspaceToAllocate()
                                            - magnitudeTolerance);
                            System.out
                                    .printf("A nodeAllocated %d node %s ringspaceAllocated %d\n",
                        ...
                        if (protoRegion.getRegion().getSize() <= NumUtil.addWithClamp(nas.allocationRemaining(replicaIndex), magnitudeTolerance)) {
                            ...
                            protoRegion = prList.get(regionIndex);
                            if (protoRegion.getRegion().getSize() != nas
                                    .allocationRemaining(replicaIndex)) {
                                throw new RuntimeException("panic");
                            }
                        }
                        ...
                        if (debug) {
                            System.out
                                    .printf("B nodeAllocated %d node %s ringspaceAllocated %d\n",
                                            nas.getAllocated(replicaIndex),
                                            node, ringspaceAllocated);
     ...
     *         the requirements of the recipe
     */
    private TopologyRing _create(SingleRing sourceRing, RingTreeRecipe recipe,
            String ringParentID) {
    ...
    private ProtoRegionList allocateSubPolicy(ProtoRegionList prList, SubPolicy subPolicy,
        ...
        parent = recipe.topology.getNodeByID(ringParentID);
        for (SubPolicyMember member : subPolicy.getMembers()) {
            prList = allocateSubPolicyMember(prList, member, recipe, parent,
                    subPolicy.getReplicationType());
        }
        return prList;
    }

    private ProtoRegionList allocateSubPolicyMember(ProtoRegionList prList,
            SubPolicyMember member, RingTreeRecipe recipe, Node parent,
    ...
    public TopologyRing reduceDataMovement(TopologyRing oldRing,
            TopologyRing newRing, RingTreeRecipe recipe) {
        SingleRing _old;
        SingleRing _new;
        SingleRing _ring;
        ...
        _ring = new SingleRing(_new.getNodeClass(), _new.getVersion(),
                _new.getStoragePolicyName());
        while (_new.getMembers().iterator().hasNext()) {
            ...
            ir = RingRegion.intersect(oEntry.getRegion(),
                    shifted_nEntry.getRegion());
            ...
            // bbbb (shifted_nEntry)
            {
                ...
                r2 = new RingRegion(LongRingspace.nextPoint(shifted_nEntry
                        .getRegion().getEnd()), oEntry.getRegion().getEnd());
            ...
            // r1 r2
            {
                ...
                r1 = oEntry.getRegion();
                r2 = new RingRegion(LongRingspace.nextPoint(oEntry.getRegion()
                        .getEnd()), shifted_nEntry.getRegion().getEnd());
     ...
     * 
     * @param _old
     *            the TopologyRing in which to look
     * @param nEntry
     *            the entry for which to find a match
    ...
    private RingEntry findBestRegionFor(TopologyRing _old, RingEntry nEntry) {
        ...
        for (RingEntry oEntry : _old.getMembers()) {
            ...
            if (difference < bestDifference) {
                ...
                bestSizeDiff = Math.abs(bestEntry.getRegion().getSize()
                        - nEntry.getRegion().getSize());
                curSizeDiff = Math.abs(oEntry.getRegion().getSize()
                        - nEntry.getRegion().getSize());
                if (curSizeDiff < bestSizeDiff) {
                    bestEntry = oEntry;
                } else if (curSizeDiff == bestSizeDiff) {
                    if (bestEntry.getRegion().getStart() != nEntry.getRegion()
                            .getStart()
                            && oEntry.getRegion().getStart() == nEntry
                                    .getRegion().getStart()) {
                        bestEntry = oEntry;
                    }
                }
    ...
    public static TopologyRing removeExcludedMembers(TopologyRing oldRing,
            Set<Node> exclusionSet) throws InvalidRingException {
        SingleRing _new;
    ...
    public static TopologyRing removeExcludedMembers(TopologyRing oldRing,
            ExclusionSet exclusionSet) throws InvalidRingException {
        return removeExcludedMembers(oldRing,
                dummySetNodes(exclusionSet.getServers()));
    }
    ...
    public static void main(String[] args) {
        try {
            if (args.length != 8) {
                System.out
                        .println("<topologyFile> <weightSpecsFile> <exclusionList> <nodeID> <storagePolicyGroup> <policyID> <HostGroupTableFile> <HostGroup,...>");
            } else {
                ...
                policyID = args[5];
                storagePolicyGroup = new PolicyParser().parsePolicyGroup(
                        storagePolicyGroupFile, VersionedDefinition.NO_VERSION);
                topology = TopologyParser.parse(topologyFile);
                topologyRingCreator = new TopologyRingCreator();

                hostGroupTable = HostGroupTable.parse(args[6],
                        VersionedDefinition.NO_VERSION);
                hostGroups = ImmutableSet.copyOf(args[7].split(","));

                recipe = new RingTreeRecipe(
                        topology,
                        topology.getNodeByID(nodeID),
                        new WeightSpecifications(VersionedDefinition.NO_VERSION)
                                .parse(weightSpecsFile), ExclusionSet
                                .parse(exclusionFile), storagePolicyGroup,
                        policyID, hostGroupTable, hostGroups, 
                        0L, // version not
                                            // used for
                                            // this tree
                ...
                System.out.println("\n\n");
                topologyRing = topologyRingCreator.reduceDataMovement(
                        topologyRing, topologyRing, recipe);
                System.out.println(topologyRing);
            }
        } catch (Exception e) {
