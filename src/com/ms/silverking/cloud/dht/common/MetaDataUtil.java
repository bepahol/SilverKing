...
public class MetaDataUtil {
    ...
    public static boolean isSegmented(byte[] storedValue, int baseOffset) {
        return ArrayUtil.equals(storedValue, baseOffset + creatorOffset, 
                MetaDataConstants.segmentationBytes, 0, MetaDataConstants.segmentationBytes.length);
    }
    ...
    public static boolean isSegmented(ByteBuffer buf) {
        ...
        return BufferUtil.equals(buf, creatorOffset, 
                MetaDataConstants.segmentationBytes, 0, MetaDataConstants.segmentationBytes.length);
    }    
    ...
    public static byte[] getUserData(byte[] storedValue, int baseOffset) {
        ...
        if (userDataLength > 0) {
            System.arraycopy(storedValue, baseOffset + getUserDataOffset(storedValue, baseOffset), 
                                    userData, 0, userDataLength);
            return userData;
        } else {
            return null;
        }
    ...
    public static int getStoredLength(ByteBuffer storedValue, int baseOffset) {
        //System.out.println("\tgetStoredLength\t"+ storedValue +"\t"+ baseOffset +"\t"+ storedLengthOffset +"\t"+ storedValue.getInt(baseOffset + storedLengthOffset));
